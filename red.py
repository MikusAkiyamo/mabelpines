...
description = "Mabel Pines - Многофункциональный бот для Discord."
...
def initialize(bot_class=Bot, formatter_class=Formatter):
    ...
    async def on_ready():
        ...
        print("-----------------")
        print("Mabel Pines - Discord Bot")
        print("-----------------")
        print(str(bot.user))
        print("\nConnected to:")
        print("{} серверов".format(servers))
        print("{} каналов".format(channels))
        print("{} пользователей\n".format(users))
        prefix_label = 'Префикс'
        if len(bot.settings.prefixes) > 1:
            prefix_label += 'ы'
        print("{}: {}".format(prefix_label, " ".join(bot.settings.prefixes)))
        print("Владелец: " + str(owner))
        print("{}/{} активных плагинов, с {} командами".format(
            len(bot.cogs), total_cogs, len(bot.commands)))
        print("-----------------")

        if bot.settings.token and not bot.settings.self_bot:
            print("\nИспользуйте эту ссылку, чтобы пригласить бота:")
            url = await get_oauth_url()
            bot.oauth_url = url
            print(url)

        print("\nОф. сервер: https://discord.gg/Da23xmN")
